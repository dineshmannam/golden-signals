...
- Deploy tooling: `bootstrap.sh` (APIs + state bucket + provisioning SA + Service Networking service agent, `--destroy` teardown), `scripts/create-triggers.sh` (2nd-gen Cloud Build infra/app triggers), `cloudbuild-infra.yaml` (Terraform pipeline). Full flow in `DEPLOY.md`. Shell scripts must stay `shellcheck`-clean and run on macOS bash 3.2 (guard empty-array expansion with `${arr[@]+"${arr[@]}"}`). The operator runs them — no GCP auth here.
- `bootstrap.sh` provisions the **Service Networking service agent** (`gcloud beta services identity create` + `roles/servicenetworking.serviceAgent` on `service-<projectNumber>@service-networking.iam.gserviceaccount.com`). Without it, Private Services Access (the Cloud SQL VPC peering in `network.tf`) fails on locked-down projects with a `compute.globalAddresses.list` permission error.
- **Teardown lifts `deletion_protection` with an apply, not a one-shot destroy.** GKE + Cloud SQL both bind the single `var.deletion_protection`; `terraform apply -var deletion_protection=false` must land *before* `terraform destroy` — `destroy -var deletion_protection=false` alone does NOT work (the flag only clears once the apply updates the resources). Cloud SQL also pins `edition = "ENTERPRISE"` in `cloudsql.tf` because the custom `db-custom-*` tier is rejected on the default ENTERPRISE_PLUS.
...
- **SLOs are a two-phase apply, gated by `var.create_slos` (default `false`).** Their SLIs filter on Prometheus series that only exist once the app is deployed and has served traffic, so creating them at infra time fails with `Cannot find metric(s)`. The `count = var.create_slos ? 1 : 0` guard covers the custom services, both gateway SLOs (`monitoring.tf`), the fulfillment SLO (`monitoring_fulfillment.tf`), every burn-rate alert, and the SLO dashboard tiles. The `*_slo_name` locals are guarded to `""` when off and only consumed by gated resources — index them `[0]`. Deploy order is in `DEPLOY.md`: Step 1 infra (off) → Step 2 app+traffic → Step 3 `apply -var create_slos=true`.
